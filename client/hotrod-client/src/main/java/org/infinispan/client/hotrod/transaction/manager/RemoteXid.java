...
public final class RemoteXid extends XidImpl {
   ...
   public void writeTo(Transport transport) {
      transport.writeSignedVInt(FORMAT_ID);
      byte[] rawData = rawData();
      transport.writeArray(rawData, globalIdOffset(), globalIdLength());
      transport.writeArray(rawData, branchQualifierOffset(), branchQualifierLength());
   }

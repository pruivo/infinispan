...
public class TxFunctionalTest<K, V> extends MultiHotRodServersTest {
   ...
   public void testSimpleTransaction(Method method) throws Exception {
      ...
      TransactionalRemoteCache<K, V> remoteCache = remoteCache(0);
   ...
   public void testConflictWithUpdateNonExisting(Method method) throws Exception {
      ...
      TransactionalRemoteCache<K, V> remoteCache = remoteCacheWithForceReturnValue(0);
      final TransactionManager tm = remoteCache.getTransactionManager();
   ...
   protected final TransactionalRemoteCache<K, V> remoteCache(int index) {
      return (TransactionalRemoteCache<K, V>) client(index).getTransactionalCache(cacheName());
   }

   protected final TransactionalRemoteCache<K, V> remoteCacheWithForceReturnValue(int index) {
      return (TransactionalRemoteCache<K, V>) client(index).getTransactionalCache(cacheName(), true);
   }

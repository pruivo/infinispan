...
public class L1GMUContainer {
   ...
   public final InternalCacheEntry getValidVersion(Object key, EntryVersion txVersion, Collection<Address> readFrom) {
   ...
   public final void insertOrUpdate(Object key, InternalCacheEntry value) {
      L1VersionChain versionChain = l1Container.get(key);
      if (versionChain == null) {
   ...
   private static class L1VersionBody extends VersionBody<L1Entry> {
      ...
      public boolean isOlder(VersionBody<L1Entry> otherBody) {
         ...
         return isOlder(thisEntry.getValue().getMetadata().version(), otherEntry.getValue().getMetadata().version());
      }
      ...
      public boolean isEqual(VersionBody<L1Entry> otherBody) {
         ...
         return isEqual(thisEntry.getValue().getMetadata().version(), otherEntry.getValue().getMetadata().version());
      }
   ...
   private class L1Entry {
      ...
      public L1Entry(InternalCacheEntry gmuCacheEntry) {
         GMUMetadata metadata = (GMUMetadata) gmuCacheEntry.getMetadata();
         this.value = gmuCacheEntry;
         this.creationVersion = metadata.creationVersion();
         this.readVersion = metadata.maximumValidVersion();
         this.invalid = false;
      }

...
@MBean(objectName = "L1GMUContainer", description = "Component that handles distribution of content across a cluster")
public class L1GMUContainer {
		...
		if (versionChain != null) {
			...
			if (versionEntry.isFound()) {
				...
				if (isValid(l1Entry, txVersion, owner)) {              
					...
					EntryVersion updatedTxVersion = checkMaxVCCondition(l1Entry,txVersion,readFrom) ? gmuVersionGenerator.mergeAndMax(txVersion, l1Entry.getReadVersion()): gmuVersionGenerator.mergeAndMax(txVersion, l1Entry.getCreationVersion());
	...
	public final boolean checkMaxVCCondition(L1Entry l1Entry, EntryVersion txVersion, Collection<Address> readFrom){	   
		for(Address a : readFrom){
			long entryVersionValue = toGMUVersion(l1Entry.getReadVersion()).getVersionValue(a);
			long txEntryVersionValue = toGMUVersion(txVersion).getVersionValue(a);
			if(entryVersionValue > txEntryVersionValue)
				return false;	   
		}
		return true;
	}
	...
	private class L1Entry {
		...
		public L1Entry(InternalGMUCacheEntry gmuCacheEntry) {
			...
			if(gmuCacheEntry.getMaximumValidVersion() != null)

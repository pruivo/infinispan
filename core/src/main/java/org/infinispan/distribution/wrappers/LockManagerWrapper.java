...
public class LockManagerWrapper implements LockManager {
   ...
   public LockManagerWrapper(LockManager actual, StreamLibContainer streamLibContainer, Configuration configuration) {
      this.actual = actual;
      this.streamLibContainer = streamLibContainer;
      this.configuration = configuration;
      this.sampleHoldTimes = configuration.customStatsConfiguration().isSampleServiceTimes();
   }
   ...
   private void flushPendingLocksIfNeeded(GlobalTransaction lockOwner) {
      if (sampleHoldTimes && GmuStatsHelper.maybePendingLocks(lockOwner))
         TransactionsStatisticsRegistry.flushPendingLocksIfNeeded(lockOwner);
   }

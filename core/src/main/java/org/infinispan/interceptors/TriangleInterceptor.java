...
public class TriangleInterceptor extends DDAsyncInterceptor {
   ...
   public void inject(RpcManager rpcManager, CommandsFactory commandsFactory, CommandAckCollector commandAckCollector,
         ...
         @ComponentName(value = KnownComponentNames.TIMEOUT_SCHEDULE_EXECUTOR) ScheduledExecutorService timeoutExcecutor) {
      this.rpcManager = rpcManager;
      this.commandsFactory = commandsFactory;
      this.commandAckCollector = commandAckCollector;
      this.distributionManager = distributionManager;
      this.timeoutExecutor = timeoutExcecutor;
   }
   ...
   private void sendPutMapBackupAck(PutMapCommand command, Collection<Integer> segments) {
      ...
      if (id.getAddress().equals(localAddress)) {
         ...
         rpcManager.sendTo(id.getAddress(),
               commandsFactory.buildBackupPutMapAckCommand(id, segments, command.getTopologyId()), DeliverOrder.NONE);
      }
   }

   private void sendPrimaryPutMapAck(PutMapCommand command, Collection<Integer> segments,

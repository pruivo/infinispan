...
public class ActionSequencer {
   ...
   public ActionSequencer(ExecutorService executor, TimeService timeService) {
   ...
   public <T> CompletionStage<T> orderOnKeys(Collection<?> keys, Callable<? extends CompletionStage<T>> action) {
      ...
      if (dKeys.length == 1) {
         entry = new SingleKeyNonBlockingSequenceEntry<>(action, executor, dKeys[0], statCollector);
      } else {
         entry = new MultiKeyNonBlockingSequenceEntry<>(action, executor, dKeys, statCollector);
      }
   ...
   public <T> CompletionStage<T> orderOnKey(Object key, Callable<? extends CompletionStage<T>> action) {
      checkAction(action);
      StatCollector statCollector = newStatCollector();
      SequenceEntry<T> entry = new SingleKeyNonBlockingSequenceEntry<>(action, executor, checkKey(key), statCollector);
      registerAction(entry);
      return entry;
   }
   ...
   private StatCollector newStatCollector() {
      return collectStats ? new StatEnableCollector() : NO_STATS;
   }
   ...
   private class SingleKeyNonBlockingSequenceEntry<T> extends SequenceEntry<T> {
      ...
      SingleKeyNonBlockingSequenceEntry(Callable<? extends CompletionStage<T>> action, ExecutorService executor,
            Object key, StatCollector statCollector) {
         super(action, executor, statCollector);
         this.key = key;
      }
   ...
   private class MultiKeyNonBlockingSequenceEntry<T> extends SequenceEntry<T> {
      ...
      MultiKeyNonBlockingSequenceEntry(Callable<? extends CompletionStage<T>> action, ExecutorService executor,
            Object[] keys, StatCollector statCollector) {
         super(action, executor, statCollector);
         this.keys = keys;
      }
   ...
   private class StatEnableCollector extends StatCollector {

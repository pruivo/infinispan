...
public class InvalidateL1Command extends InvalidateCommand {
   ...
   public InvalidateL1Command(DataContainer dc, DistributionManager dm, CacheNotifier notifier, Set<Flag> flags,
                              CommandUUID commandUUID, Object... keys) {
      super(notifier, flags, commandUUID, keys);
      writeOrigin = null;
      this.dm = dm;
   ...
   public InvalidateL1Command(DataContainer dc, DistributionManager dm, CacheNotifier notifier, Set<Flag> flags,
                              Collection<Object> keys, CommandUUID commandUUID) {
      this(null, dc, dm, notifier, flags, keys, commandUUID);
   }

   public InvalidateL1Command(Address writeOrigin, DataContainer dc, DistributionManager dm, CacheNotifier notifier,
                              Set<Flag> flags, Collection<Object> keys, CommandUUID commandUUID) {
      super(notifier, flags, keys, commandUUID);
   ...
   public Object[] getParameters() {
      if (keys == null || keys.length == 0) {
         return new Object[]{commandUUID, writeOrigin, 0};
      } else if (keys.length == 1) {
         return new Object[]{commandUUID, writeOrigin, 1,  keys[0]};
      } else {
         Object[] retval = new Object[keys.length + 3];
         int i = 0;
         retval[i++] = commandUUID;

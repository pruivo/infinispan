...
public class RpcManagerImpl implements RpcManager, JmxStatisticsExposer {
   ...
   public void sendTo(Address destination, ReplicableCommand command, DeliverOrder deliverOrder) {
      if (trace) {
         log.tracef("%s invoking %s to %s order by %s", t.getAddress(), command, destination, deliverOrder);
      }
   ...
   public void sendTo(ReplicableCommand command, DeliverOrder deliverOrder, Collection<Address> destinations) {
      if (trace) {
         log.tracef("%s invoking %s to list %s order by %s", t.getAddress(), command, destinations, deliverOrder);
      }
   ...
   private <T> T errorReplicating(Exception e) {
      log.unexpectedErrorReplicating(e);
      if (statisticsEnabled) replicationFailures.incrementAndGet();
      return rethrowAsCacheException(e);
   }

...
public class CommandAwareRpcDispatcher extends MessageDispatcher {
   ...
   private static final int REPLY_FLAGS_TO_CLEAR =
         Message.Flag.RSVP.value() | Message.Flag.INTERNAL.value() | Message.Flag.NO_TOTAL_ORDER.value();
   ...
   public CommandAwareRpcDispatcher(JChannel channel, JGroupsTransport transport,
         InboundInvocationHandler globalHandler, ScheduledExecutorService timeoutExecutor, TimeService timeService,
      ...
      correlator(new CustomRequestCorrelator(prot_adapter, this, local_addr, ispnMarshaller));
   }
   ...
   private void reply(org.jgroups.blocks.Response response, Object retVal, ReplicableCommand command, Message req) {
      if (response != null) {
         ...
         try {
            rsp_buf = new Buffer(ispnMarshaller.objectToByteBuffer(retVal));
         } catch (Throwable t) {
            try {
               // this call should succeed (all exceptions are serializable)
               rsp_buf = new Buffer(ispnMarshaller.objectToByteBuffer(t));
               is_exception = true;
   ...
   Buffer marshallCall(ReplicableCommand command) {
      try {
         return new Buffer(ispnMarshaller.objectToByteBuffer(command));
   ...
   private void processCallsStaggered(ReplicableCommand command, RspFilter filter, List<Address> dests,
      ...
      if (subFuture != null) {
         subFuture.whenComplete((rsp, throwable) -> {
            ...
            if (rsp.wasReceived() && (filter == null || filter.isAcceptable(rsp.getValue(), dest))) {

...
public class IracLocalStateTransferTest extends AbstractXSiteTest {
   ...
   public void testStateTransfer(Method method) {
      ...
      changeOwnership(LON, NUM_NODES, 1); //primary owner changes from node-0 to node-3
      addNewNode(site(LON));
      site(LON).waitForClusterToForm(null);

      assertOwnership(LON, key, 3, 1);
   ...
   public void testBackupSendAfterPrimaryFail(Method method) {
      ...
      assertInSite(NYC, cache -> AssertJUnit.assertNull(cache.get(key)));
   ...
   public void testBackupRemovedKeySendAfterPrimaryFail(Method method) {
      ...
      assertInSite(NYC, cache -> AssertJUnit.assertEquals(value, cache.get(key)));
   ...
   public void createBeforeMethod() {
      ...
      changeOwnership(LON, 0, 1);
      changeOwnership(NYC, 0, 1);
   ...
   protected void createSites() {
      GlobalConfigurationBuilder lonGCB = globalConfigurationBuilderForSite(LON);
      ...
      GlobalConfigurationBuilder nycGCB = globalConfigurationBuilderForSite(NYC);
   ...
   private void assertOwnership(String site, String key, int primaryOwner, int backupOwner) {
      AssertJUnit.assertTrue(getDistributionForKey(cache(site, primaryOwner), key).isPrimary());
      AssertJUnit.assertTrue(getDistributionForKey(cache(site, backupOwner), key).isWriteBackup());
   }
   ...
   private void addNewNode(TestSite site) {
      ...
      GlobalConfigurationBuilder gBuilder = globalConfigurationBuilderForSite(siteName);
      ConfigurationBuilder builder = LON.equals(siteName) ?
                                     getLonActiveConfig() :
                                     getNycActiveConfig();
      site.addCache(gBuilder, builder);
   }

   private void changeOwnership(String site, int primaryOwner, int backupOwner) {
      ControlledConsistentHashFactory<?> chf = LON.equals(site) ?
                                               lonCHF :
                                               nycCHF;
      chf.setOwnerIndexes(primaryOwner, backupOwner);
   }

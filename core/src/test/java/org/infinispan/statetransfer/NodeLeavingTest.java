...
/**
 * Tests the read consistency when a node leaves the cluster.
 *
 * @author Pedro Ruivo
 * @since 8.0
 */
...
public class NodeLeavingTest extends MultipleCacheManagersTest {
   ...
   public void testNodeLeaving(Method method) throws InterruptedException, ExecutionException, TimeoutException {
      ...
      final ControllerTransport transport = wrapGlobalComponent(container, Transport.class,
      ...
      Future<?> join = fork(() -> container.getCache(CACHE_NAME));
      ...
      assertData(method, cache(0, CACHE_NAME), cache(2, CACHE_NAME), cache(3, CACHE_NAME));
   }
   ...
   private void assertData(Method method, Cache<?, ?>... caches) {

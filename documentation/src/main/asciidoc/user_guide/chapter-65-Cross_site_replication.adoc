...
NOTE: This feature is available since Infinispan *7.0.0.Alpha1*

CAUTION: This is an experimental feature. Please check the release notes for more information about what is implemented.
...
can be found over the +XSiteAdminOperations+ managed bean and it is named +pushState(siteName)+. The system
administrator should invoke this operation in the provider site (i.e. the site that will send the state) and he/she
sets the name of the consumer site (i.e. the site that will receive the state).

*I NEED A FANCY IMAGE TO HERE*
...
can be overwritten but they are never delete. In other words, if a key +K+ does not exists in the provider site but it
exists in consumer site, it will not be delete. In other way, if a key +K+ exists in both sites, it will be overwritten
...
<namedCache name="xSiteStateTransfer">
   ...
    <sites>
        <backups>
           <backup site="NYC" strategy="SYNC" backupFailurePolicy="FAIL">
               <stateTransfer chunkSize="512" timeout="1200000" />
           </backup>
        </backups>
    </sites>
    ...
...
This release only contains the basic behavior to transfer the state. It's assumed that no nodes will join or leave (crash) either
in local or backup site. Also, it assumes a reliable links between sites.

For the moment, it is not possible to cancel the state transfer.
